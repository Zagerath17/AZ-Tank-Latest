...
// can hold dev powers — the tools panel, the free Elo/tag adjusters,
// all of it is unreachable. Kept as a stub returning false so every
// caller keeps working and nothing can quietly re-enable it.
function isDevEmail() {
  return false;
}
...
async function adoptProfile(uid, wantName = null, email = null) {
  ...
  // ---- ONE-TIME WIPE ----------------------------------------------
  // A single clean slate: both ratings reset to 500, every tag spent,
  // every bought paint and pattern removed, and any dev marker
  // stripped. Each account applies this once, the first time it signs
  // in after the update, and is stamped so it never happens again.
  // Bump WIPE_VERSION to run another one later.
